...
| 18 | provider | The brain is **provider-switchable** (Anthropic ↔ OpenAI) in both harnesses: the `--text` REPL picks via `--provider anthropic\|openai [--model <id>]`, the production pipeline via `config.assistantProvider`. Tool schemas/impls/system prompt are shared; only request/response shape differs. GPT-5.6 needs `reasoning_effort="none"` for chat-completions tool calls (also what we want — no reasoning for a one-sentence answer). Needs `openaiApiKey` in secrets.json | User wants an easy Haiku-vs-GPT-5.6 A/B; Pipecat already abstracted the pipeline, only the raw-Anthropic REPL was locked | KEY: `openaiApiKey`, then `--text --provider openai` vs `--provider anthropic` on the 20-query set | |
