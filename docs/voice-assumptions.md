...
| 19 | audio | **Bluetooth headsets (AirPods) are a degraded test rig, not a target.** Our per-wake stream churn (wake mic ↔ session mic+speaker) forces HFP↔A2DP profile renegotiation — audible connect/disconnect flapping, and occasional `-9999 Unanticipated host error` on a stream open mid-renegotiation (session goes silent, idle-times out gracefully, wake again). Wake-tick playback retries once; beyond that we deliberately don't engineer around BT — wired/USB audio (the array) has none of this | Architecture targets the wired ReSpeaker; Windows BT telephony-mode is inherently flappy under duplex churn | Wired mic/speaker (or the array) makes it vanish; observed live on AirPods 2026-08-10 | |
