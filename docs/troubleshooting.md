...
| Launch fails `host never reported READY` after ~2 min, TV never came on | The set acked `power_on` and did not wake. Ex-Link is send-only so nothing on either machine can see this — `enter_died` + `enter_redispatched` on the turn means the K15 caught Enter's exit and asked again; both present with still no `host_ready` means the TV refused twice | Switch it on by hand and re-chord. Standing hole: measurements and the two open leads are in [tv-power-detection-design.md](tv-power-detection-design.md). **Do not re-run the Ex-Link ack probe** — it is answered there |
