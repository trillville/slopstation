...
| Listener says `armed` but the chord never fires | **`chord_partial` first** — it carries the byte the press actually produced (`btn`) against the one needed (`want`). Partials present → buttons arrive, so the press was wrong or held under 2 s. `armed` with no partials at all → the Puck enumerates and rumbles but reports nothing (`main()`'s claim comment). Partials whose `btn` never covers `want` → a firmware update moved the report bytes | Bytes moved: `python calibrate.py` (**stop the listener first** — one process owns the Puck), update `RID_INPUT`/`BTN_BYTE`/`CHORD` in `chord_listener.py`, then `.\Start-K15.bat`. No partials at all: reseat the Puck's USB or restart VirtualHere |
| Launch fails `TV never reported on` (~60–90 s: Enter's own death plus the rescue's bounded wait) | The set acked `power_on` and kept **answering** not-on the whole way — the K15 asked the TV itself throughout the READY wait (`tv_poll` in couch.py, needs `tvIp`), re-poked it every ~6 s, and after `enter_died` refused to spend the one redispatch on a dark panel. This is the honest, named form of the old 2-minute `host never reported READY` death; a healthy launch pays nothing for it (Enter is never gated on the TV) | Switch it on by hand and re-chord. Making the refusal rarer is README § Still genuinely open (standby-depth settings, a WoL second channel). **Do not re-run the Ex-Link ack probe** — the ack is a constant; couch.py's launch_start comment carries the measurement |
| Launch fails `host never reported READY` after ~2 min, TV never came on | A rig without `tvIp` (or a set unreadable over the LAN — check `tv_state_unknown` on the turn) still cannot see the panel: Ex-Link is send-only. `enter_died` + `enter_redispatched` on the turn means the K15 caught Enter's exit and asked again; both present with still no `host_ready` means the TV refused twice | Switch it on by hand and re-chord. Set `tvIp` so the launch can ask the set directly and fail in 30 s with the TV named instead |
