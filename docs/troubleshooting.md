...
| Launch fails `TV never reported on` after ~30 s of polling (measured from the PC answering ssh — a cold boot adds its own time first) | The set acked `power_on` and kept **answering** standby — the K15 asked the TV itself (`wait_tv_on` in couch.py, needs `tvIp`) and it refused several frames. This is the honest early form of the old 2-minute `host never reported READY` death | Switch it on by hand and re-chord. Making the refusal rarer is README § Still genuinely open (standby-depth settings, a WoL second channel). **Do not re-run the Ex-Link ack probe** — the ack is a constant; couch.py's launch_start comment carries the measurement |
