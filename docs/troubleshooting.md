...
| Controller connects but buttons do nothing (Steam button may still work, haptics alive) | The K15 was still reading the Puck while the PC claimed it, so VirtualHere unbound the device out from under 13 live readers. Listener lane shows `puck_vanished reason=claimed` instead of `puck_standoff` | End the session and relaunch — the claim is already wedged, nothing recovers it in place. Then check the listener let go *before* the claim: a healthy launch logs `puck_standoff`, never `puck_vanished`. If it's still `puck_vanished`, the standoff isn't running — `.\Start-K15.bat` to reload the listener onto current code |
