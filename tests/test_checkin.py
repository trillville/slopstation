"""Blind test: cron check-ins - the URL built out of the DSN, the upsert
...
def test_checkin():
    ...
    assert checkin.parse_dsn(
        "https://k0000000000000000000@o1.ingest.sentry.io/42")[0] \
        == "o1.ingest.sentry.io"
    print(f"  dsn: host/project/key out of {host}")

    # Absent, template junk, and things that are not DSNs all read as absent -
    # the same gate every other keyed lane uses.
    for junk in (None, "", "https://...@o1.ingest.sentry.io/1", 12345,
    ...
    print("  gate: placeholders, wrong scheme, missing key/project all absent")

    # -- the check-in URL ------------------------------------------------------
    url = checkin.checkin_url(DSN, "listener")
    assert url == ("https://o4509876.ingest.us.sentry.io/api/1234567/cron/"
                   "k15-listener/abc123def456abc123def456abc12345/"), url
    ...
    print(f"  url: {url.rsplit('/', 3)[0]}/<slug>/<key>/")
    ...
    body = json.loads(json.dumps({"monitor_config": checkin.MONITOR_CONFIG,
                                  "status": "ok"}))
    ...
    print("  payload: interval schedule, 2-miss threshold, margin "
          f"{checkin.MONITOR_CONFIG['checkin_margin']}m")
    ...
    print("  send: a dead endpoint is False, not an exception")

    # No DSN and a test run both mean "do not start". Under the blind suite
    ...
    print("  start: no DSN and env=test both no-op")
    ...
    def records():
        try:
            return [json.loads(x) for x in
                    stream.read_text(encoding="utf-8").splitlines() if x.strip()]
        except OSError:
            return []
    ...
    print("  thread: daemon, first result emitted as checkin_failed(monitor=)")

    print("OK - checkin: dsn parse, per-lane url, upsert payload, fail-soft, "
          "env gate, one event per transition")
