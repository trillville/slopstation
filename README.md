...
| `voice/` | The voice overlay: `voice_agent.py` (wake word → per-session Pipecat pipeline: Flux STT → grammar gate → dispatch, with an LLM assistant lane), `grammar.yaml` + `titles.py` (command grammar + fuzzy title resolution), `dispatch.py` (every voice side effect, shared by both lanes), `preroll.py` (no-pause wake buffer), `assistant.py` (catalog-in-context brain, optional web search, + `--text` REPL), `workers.py`/`jobs.py`/`announce.py` + `worker_home/` (Tier-3 background tasks: claude/codex CLI adapters, job store, proactive spoken results), `tests/` (blind suite). Own venv, own pins. |
