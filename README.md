...
The TV is told its input up front, the way a console's HDMI-CEC does it,
because the PC can only see the TV on the TV's active input. A failed launch
that woke the TV turns it back off. Controller input and voice run in separate
processes, so either can restart independently.
