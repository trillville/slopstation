...
def check_config():
    try:
        ...
        report(FAIL, "config.json", f"unreadable ({e})", "recreate from config.example.json")
        return None
    missing = cglib.missing_config(cfg)
    n = len(cglib.REQUIRED_CONFIG)
    if missing:
        report(FAIL, "config.json", f"missing keys: {missing}", "compare with config.example.json")
    else:
...
def _steam_mint_probe(days):
    ...
    try:
        p = subprocess.run([sys.executable, "-m",
                            "slopstation.agent.tools.steam_session", "token"],
...
def check_voice_venv(cfg):
    ...
    if (venv / "deps-ok").exists():
        ...
        vendored = (pathlib.Path(__file__).resolve().parent / "agent"
                    / "models" / f"{model}.onnx")
        onnx = (venv / "Lib" / "site-packages" / "openwakeword"
        ...
        report(WARN, "voice venv", "not bootstrapped (no deps-ok sentinel)",
