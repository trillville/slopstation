...
def open_streaming_interface(accept=None, timeout_s: float = 2.0):
    ...
    hid is imported here, not at module scope: this module is on the chord
    lane's import path and must load on system python with no hidapi."""
    import hid
...
def chirp(dev, gain: int = 0, write=None) -> None:
    ...
    for freq, dur in ((440, 60), (660, 90)):
        for side in (0, 1):
            put(dev, tone_report(side, freq, dur, gain), f"tone {freq}Hz/{dur}ms side{side}")
        time.sleep(0.07)
    for side in (0, 1):
