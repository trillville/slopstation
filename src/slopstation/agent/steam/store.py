...
def store_items(appids: list[int]) -> dict:
...
def fetch_wishlist_on_sale(steamid: str) -> list[dict]:
    """The wishlist's discounted games, best first."""
    on_sale = [g for g in fetch_wishlist(steamid) or [] if g.get("discount", 0) > 0]
...
def fetch_store_search(term="", tags=None, max_price=None, on_sale=False):
...
def fetch_release(data: dict) -> dict | None:
    if not data:
        return None
...
def fetch_featured(section: str) -> list[dict] | None:
    """One of the store's front-page feeds: new_releases, top_sellers,
    coming_soon, or the specials refresh_deals keeps. Curated by Steam, a
    couple dozen each. None when the store did not answer, so an outage is
    never read as an empty feed."""
    d = _get(f"{STORE}/api/featuredcategories", {"cc": _cc(), "l": "english"})
    if d is None:
        return None
...
def fetch_wishlist(steamid: str) -> list[dict] | None:
    ...
    priced = store_items([int(it["appid"]) for it in items])
    return [
        {
            "appid": int(it["appid"]),
...
def refresh_deals() -> int:
    ...
    specials = fetch_featured("specials") or []
