...
Heavy imports (pipecat, provider SDKs) stay INSIDE run_session so importing
...
async def run_session(cfg, secrets, matcher, args, input_idx, output_idx,
    ...
    stt = DeepgramFluxSTTService(
        ...
        settings=DeepgramFluxSTTService.Settings(
            ...
            # InterimTranscriptionFrame, which GrammarGate (finals only) never
            # sees. Its one live effect is resetting the idle clock.
    ...
    # No barge-in: nothing here constructs an InterruptionWorkerFrame and the
    # transport has no vad_analyzer, so a Tier-1 command spoken mid-answer
    # DISPATCHES while the answer keeps playing over it.
    #
    ...
    worker = PipelineWorker(
        ...
        # Pipecat's conversation id IS our session id: a Langfuse trace and
        # the JSONL lines around it join on this value.
    ...
    async def _on_idle(worker):
        ...
        # blocks, so the idle clock can expire mid-utterance or mid-ssh.
        if gate.is_busy():
    ...
    if capture is not None:
        # Stop as late as possible: only worker start + mic-open remain, so
        # the uncaptured gap is ~100-200 ms, not the whole session build.
    ...
    if context is not None:                     # cross-session follow-ups
        ...
        # Drop the seeded job results before carrying - the next session
        # re-seeds them from jobs.json, so carrying them too would double the
        # findings until they aged out. They sit on both sides of the carried
        # block; cut by the counts recorded at build time.
