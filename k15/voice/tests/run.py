...
Exit code = failures. Machine needs: steam (a local Steam + PowerShell),
audio (real devices), models (wake models fetched on first run).
"""
import os
import shutil
import subprocess
...
def detect():
    have = []
    if shutil.which("powershell") and os.environ.get("ProgramFiles(x86)"):
        try:
            import winreg
...
def main(argv):
    ...
    have = detect()
    env = dict(os.environ, CG_TEST_HAS=",".join(have))
    results = []
    for f in files:
        need = NEEDS.get(f.stem)
        if need and not run_all and need not in have:
            results.append((f.stem, "SKIP", 0.0, f"needs {need}"))
            continue
        t0 = time.time()
        ...
        if p.returncode == 0 and last.startswith("SKIP"):
            status = "SKIP"
