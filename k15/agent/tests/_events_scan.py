...
def python_files():
    yield from sorted(p for p in K15.glob("*.py") if p.name != "haptic_test.py")
    # The package walk, not rglob: tests/ and bench/ carry event names of their
    # own ("collide", "x") and a lane literal "lane", which the frozen sets
    # would then have to hold.
    yield from _bootstrap.agent_modules()
...
def scan_bat():
    out = {}
    for path in list(K15.glob("*.bat")) + list((K15 / "agent").glob("*.bat")):
